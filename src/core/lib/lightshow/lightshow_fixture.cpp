...
void LightshowFixture::set_type(std::string type) {
  ...
  if (type == "bass" || type == "mid" || type == "high" || type == "action" || type == "everything" || type == "ambient" || type == "color_change_beats" || type == "color_change_beats_action" || type == "color_change_onsets" || type == "onset_blink") {
    this->type = type;
