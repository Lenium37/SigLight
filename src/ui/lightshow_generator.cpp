...
void LightshowGenerator::generate_color_changes(std::shared_ptr<Lightshow> lightshow_from_analysis,
  ...
  std::cout << "Number of color changes in this lightshow: " << timestamps_float.size() << std::endl;

  Logger::debug("Number of color changes in this lightshow: {}", timestamps_float.size());
  ...
  for (int i = 0; i < timestamps_float.size(); i++) {
    ...
    color_changes.push_back({ timestamps_float[i], colors[c] });
  }
  this->set_hard_color_changes(lightshow_from_analysis, fix, color_changes);
}
